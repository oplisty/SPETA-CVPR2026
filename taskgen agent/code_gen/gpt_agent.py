...
def generate(message, gpt="deepseek", temperature=0):
    ...
    response = client.chat.completions.create(
        model=MODEL,
        messages=message,
        stream=False,#False: wait for full response; True: stream response
        temperature=temperature,#控制生成文本的​​随机性和创造性​ from 0.0(conservative and consistent) to 2.0(more random and creative)
    )
